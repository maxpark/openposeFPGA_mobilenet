...
#ifdef DEBUG
  ofstream cin_patch("cnn_sw_cin_patch.dat");
  for (int h = 0; h < IN_H_T + LAYER1_K - 1; h++)
    for (int w = 0; w < IN_W_T + LAYER1_K - 1; w++){
      int i = 0;
      int idx = h * LAYER1_IN_W_HW * LAYER1_IN_NUM_T + w * LAYER1_IN_NUM_T + i;
      cin_patch << cin_hw[idx] << endl;
    }
  cin_patch.close();

//  ofstream cin_patch("cnn_sw_cin_patch.dat");
//  cout << "cnn_sw: " << LAYER1_IN_H << " " << LAYER1_K << " " << LAYER1_IN_W << " " << LAYER1_IN_NUM << " " << IN_NUM_T << endl;
//  for (int h = 0; h < IN_H_T + LAYER1_K - 1; h++)
//    for (int w = 0; w < IN_W_T + LAYER1_K - 1; w++){
////      for (int i1 = 0; i1 < LAYER1_IN_NUM_HW / IN_NUM_T; i1++)
////        for (int i2 = 0; i2 < IN_NUM_T; i2++){
////          cout << "here!" << endl;
//          cin_patch << cin_hw[0 * LAYER1_IN_H_HW * LAYER1_IN_W_HW * IN_NUM_T + h * LAYER1_IN_W_HW * IN_NUM_T + 
//            w * IN_NUM_T + 0] << endl;
//        }
//  cin_patch.close();
#endif  
...
#ifdef DEBUG
  cout << "Preprocess: " << weight_hw[0] << endl;
#endif  
